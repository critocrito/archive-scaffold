...
doit() {
  "$(npm bin)"/sugarcube \
              -c pipelines/telegram_channels.json \
              -q queries/mail-recipients.json \
              -Q sheets_query:TelegramChannelsHistoric \
              --google.spreadsheet_id "$1" \
              --google.to_spreadsheet_id "$1" \
              --google.to_sheet TelegramChannelsDaily \
              -d
}

echo "Starting historic scrape of Telegram channels."
...
do
  ...
  doit "$ID" 2>&1 | tee -a ./logs/telegram-channels-historic-"$ID"-"$DATE".log
