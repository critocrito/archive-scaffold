...
LOGFILE="./logs/website-archival/$DATE-$ID.log"
...
doit() {
  "$(npm bin)"/sugarcube \
              -c pipelines/website_archival.json \
              -Q sheets_query:WebsiteArchivalIncoming \
              --google.spreadsheet_id "$1" \
              --google.to_spreadsheet_id "$1" \
              --google.to_sheet WebsiteArchivalDone \
              -d
}

echo "Starting the incoming archival of websites."
