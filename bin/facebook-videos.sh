...
doit() {
  "$(npm bin)"/sugarcube \
              -c pipelines/facebook-videos.json \
              -q queries/mail-recipients.json \
              -Q sheets_query:FacebookVideosIncoming \
              --google.spreadsheet_id "$1" \
              --google.to_spreadsheet_id "$1" \
              --google.to_sheet FacebookVideosDone \
              -d
}

echo "Starting incoming scrape of Facebook videos."
...
do
  ...
  doit "$ID" 2>&1 | tee -a ./logs/facebook-videos-"$ID"-"$DATE".log
